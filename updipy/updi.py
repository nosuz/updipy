...
if __name__ == '__main__':
    ...
    print("".join([chr(c) for c in UPDI.NVMPROG_KEY]))
    print("".join([chr(c) for c in UPDI.CHIP_ERASE_KEY]))
    print("".join([chr(c) for c in UPDI.USERROW_WRITE_KEY]))

    device = Device.select("attiny202")
    updi = UPDI("/dev/ttyUSB1", device=device)

    print(updi.get_key())
    print(updi.get_key(True))

    updi.set_key(UPDI.NVMPROG_KEY)
    updi.ldcs(device.ASI_KEY_STATUS)

    updi.req_reset()
    updi.ldcs(device.ASI_KEY_STATUS)
    count = 0
    while True:
        status = updi.ldcs(device.ASI_SYS_STATUS)
        if (status[0] & device.ASI_SYS_STATUS_NVMPROG_mask) != 0:
            print("Ready for NVMPROG")
            break
        elif count > 3:
    ...
    for addr in range(11):
        fuse = updi.lds(device.FUSES_base + addr)[0]
        print(f"Fuse {addr:X}:", f"{fuse:02X}")

    # read SIGROW
    id_check = True
    sig_byte = device.SIG_BYTES[device.DEVICE_NAME]
    for addr in range(len(sig_byte)):
        if sig_byte[addr] != updi.lds(device.SIGROW_base + addr)[0]:
            id_check = False
    if id_check:
        print("Confirmed:", device.DEVICE_NAME)

    print("Read memoy by each")
    for h in range(8):
        data = []
        for l in range(16):
            addr = (h << 4) | l
            data.append(updi.lds(0x8000 + addr)[0])
            print(f"{addr:04X}:", " ".join([f"{d:02X}" for d in data]))

    print("Read memoy by ptr")
    for h in range(8):
        addr = (h << 4)
        updi.st(UPDI.SET_PTR, 0x8000 + addr)
        data = []
        updi.repeat(15)
        data += updi.ld(UPDI.AT_PTR_INC)
        data += updi.repeat_read(15)
        print(f"{addr:04X}:", " ".join([f"{d:02X}" for d in data]))

    # write fuse
    addr = device.FUSES_base + 0x02
    # updi.sts(device.NVMCTRL_ADDRL, addr) # writing word not write high byte
    updi.sts(device.NVMCTRL_ADDRL, addr & 0xFF)
    updi.sts(device.NVMCTRL_ADDRH, addr >> 8)
    updi.sts(device.NVMCTRL_DATA, 0x02)
    updi.sts(device.NVMCTRL_CTRLA, 0x07)
